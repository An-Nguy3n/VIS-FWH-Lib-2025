...
public:
    ...
    void run();
    bool save_result(bool force_save, double *h_final);
...
Generator::Generator(string config_path){
    ...
    string command = "python " + config.lib_abs_path + "/PySources/extract_data.py "
...
void Generator::run(){
    ...
    while (true){
        command = "python " + config.lib_abs_path + "/PySources/create_table_PolyMethod.py "
        ...
        command = "python " + config.lib_abs_path + "/PySources/create_checkpoint_PolyMethod.py "
...
bool Generator::save_result(bool force_save, double *h_final){
    int i, j;
    int64_t k;
    ...
    for (i=0; i<count_temp_storage; i++){
        for (j=0; j<config.num_cycle; j++){
            if (h_final[i*config.num_cycle*num_field + j*num_field + config.eval_index]
                >= config.eval_threshold - config.threshold_delta * (double)j){
                k = current[2][0]+i;
    ...
    if (save){
        ...
        // Run python to execute query
        string command = "python " + config.lib_abs_path + "/PySources/run_query.py "
