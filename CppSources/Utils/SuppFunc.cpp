...
string init_insert_query(int num_opr, int cycle){
    string query = "insert into T" + to_string(cycle) + "_" + to_string(num_opr) + " values ";
    return query;
}


void update_insert_query(string &query, int64_t id, int num_opr, uint8_t *formula, int cols, int num_field, double *field){
    int i, temp;
    query += "(" + to_string(id) + ",";
    for (i=0; i<num_opr; i++){
        temp = formula[2*i]*cols + formula[2*i+1];
        query += to_string(temp) + ",";
    }
    for (i=0; i<num_field; i++){
        query += to_string(field[i]) + ",";
    }
    query.pop_back();
    query += "),";
}
...
void complete_insert_query(string &query){
    query.pop_back();
    query += ";";
}
